...
smart_car_ret mqtt_client_publish(char *top, int qos, char *msg, int len, mqtt_client *client_s)
{
...
/// @param client_s 
/// @return 
smart_car_ret mqtt_client_subscribe(char *top, int qos, mqtt_client *client_s)
{
    if (client_s->client == NULL)
	return -1;
...
smart_car_ret mqtt_client_open(mqtt_client *client_s, void *context)
{
    int rc;
    MQTTClient_connectOptions conn_opts = MQTTClient_connectOptions_initializer;
    MQTTClient_create(&(client_s->client), client_s->address, client_s->clientid, MQTTCLIENT_PERSISTENCE_NONE, NULL);
    MQTTClient_setCallbacks(client_s->client, context, client_s->connlost, client_s->msgarrvd, client_s->delivered);
